...
import configparser
import os, time, platform, urllib, qiniu, pyperclip, signal, threading
...
def job(file, mode):
    ...
    pyperclip.copy(url)
    pyperclip.paste()
    with open('MARKDOWN_FORMAT_URLS.txt', 'w+') as f:
        image = '![' + url + ']' + '(' + url + ')' + '\n'
        print(image)
        f.write(image + '\n')
...
try:
	...
	addr='http://opio0lr00.bkt.clouddn.com/'
	enable=True
